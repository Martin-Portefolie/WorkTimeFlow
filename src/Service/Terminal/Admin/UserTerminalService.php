...
final class UserTerminalService
{
    ...
    private const ADD_STEPS = [
        ['key'=>'email',    'label'=>"New user's email? (required)"],
        ['key'=>'username', 'label'=>"Username? (Enter to derive from email)"],
        ['key'=>'roles',    'label'=>"Roles? (comma-separated; Enter for ROLE_USER)"],
        ['key'=>'password', 'label'=>"Password? (Enter for autogenerated)"],
        ['key'=>'notify',   'label'=>"Send credentials email? (yes/no, Enter = yes)"],
    ];
    ...
    public function addCommand(array $tokens): array
    {
        ...
        // If flags were supplied, keep non-interactive behavior
        if (!empty($f)) {
    ...
    /** Old users:add body extracted here unchanged */
    private function addCommandNonInteractive(array $f): array
    {
        // Paste the body of your current addCommand here, using $f instead of $flags
        // (The long implementation you posted already does email/roles/password/etc.)
        // Return the same shapes as before.
        // --- BEGIN your existing logic (trimmed note) ---
        ...
        try { $this->em->persist($user); $this->em->flush(); }
        catch (\Throwable $e) { return ['output' => 'Failed to create user: '.$e->getMessage(), 'success' => false]; }
    ...
    private function handleAddStep(array $state, string $input, string $userId): array
    {
        $i     = (int)($state['step'] ?? 0);
        ...
        if ($i < count($steps)) {
            ...
            if ($key === 'email') {
                if ($val === '' || !filter_var($val, FILTER_VALIDATE_EMAIL)) {
                    return $this->formatPrompt("Please enter a valid email (e.g., alice@example.com).");
                }
                if ($this->users->findOneBy(['email' => $val])) {
                    return $this->formatPrompt("Email already exists. Enter a different email.");
                }
            }
        ...
        // Build user from collected data
        $d = $state['data'] ?? [];
        ...
        $plain = !empty($d['password']) ? (string)$d['password'] : bin2hex(random_bytes(4));
        ...
        $msg = sprintf(
            ...
            $user->getUsername(),
            implode(',', $user->getRoles()),
            $d['password'] ? '(set)' : '(autogenerated)',
            $sent ? 'sent' : 'not sent'
        );

        return ['output' => nl2br(htmlspecialchars($msg, ENT_QUOTES, 'UTF-8')), 'success' => true];
    }
    ...
    private function formatPrompt(string $label): array
    {
        return [
            ...
            'success' => true,
            'await'   => true, // tells the UI to accept a free-form answer next
        ];
    }
    ...
    private function currentUserKey(): string
    {
        ...
        return method_exists($u, 'getUserIdentifier') ? (string)$u->getUserIdentifier() : 'guest';
    }
