...
final class TerminalTodoService
{
    ...
    public function list(array $flags = []): array
    {
        ...
        $qb = $this->todoRepository->createQueryBuilder('todo')
        ...
        if ($q !== null && $q !== '') {
            $search = 'LOWER(todo.name) LIKE :q OR LOWER(p.name) LIKE :q';

            if (ctype_digit($q)) {
                $search = 'todo.id = :id OR ' . $search;
                $qb->setParameter('id', (int) $q);
            }

            $qb->andWhere($search)->setParameter('q', '%' . mb_strtolower($q) . '%');
        }
