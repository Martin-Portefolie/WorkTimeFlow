...
use App\Service\Terminal\Admin\TerminalTimelogService;
...
final class TerminalService
{
    ...
    private const ALIASES = [
        ...
        'timelog.l' => 'timelogs:list',
        'timelog.s' => 'timelogs:show',
        'timelog.a' => 'timelogs:add',
        'timelog.u' => 'timelogs:update',
        'timelog.del' => 'timelogs:delete',
    ];
...
//        private UserTerminalService  $userModule,
        ...
        private TerminalTimelogService $timelogModule,
    ) {}
    ...
    public function execute(string $input): array
    {
        ...
        if ($isAdmin) {
            if ($resp = $this->timelogModule->handleInteractive($raw)) { return $resp; }
        ...
        $adminCommands = [
            ...
            'timelogs:list','timelogs:show','timelogs:add','timelogs:update','timelogs:delete',
        ];
    ...
    private function handleAdmin(string $input): array
    {
        ...
        return match ($cmd) {
            'help' => [
                ...
                'vars' => [
                    'aliases' => [
                        ...
                        ['cmd' => 'timelogs:list', 'alias' => 'timelog.l', 'desc' => 'List timelog', 'usage' => '', 'flags' => ['--user=<id>, --todo=<id>, --project=<id>, --date=YYYY-MM-DD, --limit=50']],
                        ['cmd' => 'timelogs:show', 'alias' => 'timelog.s', 'desc' => 'Show timelog', 'usage' => '<id>', 'flags' => ['']],
                        ['cmd' => 'timelogs:add', 'alias' => 'timelog.a', 'desc' => 'Add timelog', 'usage' => '', 'flags' => ['--user=<id>, --todo=<id>, --date=YYYY-MM-DD, --minutes=<n>, --description=<text>']],
                        ['cmd' => 'timelogs:update', 'alias' => 'timelog.u', 'desc' => 'Update timelog', 'usage' => '<id>', 'flags' => ['--minutes=<n>, --date=YYYY-MM-DD, --description=<text>']],
                        ['cmd' => 'timelogs:delete', 'alias' => 'timelog.del', 'desc' => 'Delete timelog', 'usage' => '<id>', 'flags' => ['']],
                    ],
                ],
                'success' => true,
            ],
            ...
            'timelogs:list' => $this->withGui($this->timelogModule->list($flags), 'timelogs', 'timelogs/list'),
            'timelogs:show' => $this->withGui($this->timelogModule->show($args), 'timelogs', 'timelogs/show'),
            'timelogs:add' => $this->timelogModule->add($flags),
            'timelogs:update' => $this->timelogModule->update($args, $flags),
            'timelogs:delete' => $this->timelogModule->delete($args),
