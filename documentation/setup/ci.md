...
The `Architecture boundaries` job compares the pull request with its base and
rejects new database query construction or execution in services/controllers,
and new HTML markup generated in PHP or JavaScript. Repositories own database
queries, Twig owns HTML, and Stimulus owns events and DOM coordination. Existing
findings are reported as a baseline so this check can be adopted incrementally;
moving an old query or HTML fragment is still encouraged as separate cleanup.
